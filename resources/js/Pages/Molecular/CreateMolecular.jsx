...
const CreateMolecular = () => {
    ...
    // Update test fields on selection
    const handleTestChange = (index, field, value) => {
        const updatedFields = [...testFields];
        updatedFields[index][field] = value;

        // If test_id is selected, fetch the test fee
        if (field === "test_id") {
            const selectedTest = tests.find((t) => t.id == value);
            updatedFields[index].total = selectedTest ? selectedTest.test_fee : 0;
        }
            ...
            <form onSubmit={handleSubmit}>
                ...
                <table className="table-auto w-full mb-6 border">
                    ...
                    <tbody>
                        {testFields.map((test, index) => {
                            const selectedTest = tests.find((t) => t.id == test.test_id);
                            return (
                                <tr key={index}>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        />
                        {errors.paid && <span className="text-red-500">{errors.paid}</span>}
                    </div>
                    <div>
                        <label className="block mb-2 font-medium">Total Due</label>
                        <input
                            type="number"
                            value={totalDue}
                            readOnly
